...
function itemContentChanged()
    ...
    m.numTracks.text = trackCount.tostr() + " news"
...
function updateLayout()
    if m.top.height > 0 and m.top.width > 0 
        ...
        'The following is for showing videos quantity
        m.itemPoster.width  = posterSize 
        m.itemPoster.height = posterSize

        m.overlayBG.width  = posterSize
        m.overlayBG.height = posterSize / 4
        m.overlayBG.translation = [0, posterSize - m.overlayBG.height ]
        
        m.overlayContent.translation = [ 10, (m.overlayBG.height - m.playIcon.height) / 2 ]
    end if
end function
